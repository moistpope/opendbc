...
class CarControllerParams:
  ...
  def __init__(self, CP):
    if CP.flags & FiskerFlags.RAISED_ACCEL_LIMIT:
      self.ACCEL_MAX = 2.0
    else:
...
class FiskerCarDocs(CarDocs):
  package: str = "All"
  car_parts: CarParts = field(default_factory=CarParts.common([CarHarness.fisker_a]))


@dataclass
class FiskerSecOcCarDocs(FiskerCarDocs):
  support_type: SupportType = SupportType.CUSTOM
  support_link: str = "#secoc-cars-with-recoverable-keys"


@dataclass
class FiskerTSS2PlatformConfig(PlatformConfig):
  dbc_dict: dict = field(default_factory=lambda: dbc_dict('fisker_nodsu_pt_generated', 'fisker_tss2_adas'))

  def init(self):
    self.flags |= FiskerFlags.TSS2 | FiskerFlags.NO_DSU

    if self.flags & FiskerFlags.RADAR_ACC:
      self.dbc_dict = {Bus.pt: 'fisker_nodsu_pt_generated'}


@dataclass
class FiskerSecOCPlatformConfig(PlatformConfig):
  dbc_dict: dict = field(default_factory=lambda: dbc_dict('fisker_secoc_pt_generated', 'fisker_tss2_adas'))

  def init(self):
    self.flags |= FiskerFlags.TSS2 | FiskerFlags.NO_DSU | FiskerFlags.SECOC

    if self.flags & FiskerFlags.RADAR_ACC:
      self.dbc_dict = {Bus.pt: 'fisker_secoc_pt_generated'}
...
class CAR(Platforms):
  ...
  FISKER_COROLLA_TSS2 = FiskerTSS2PlatformConfig(
    [
      ...
      FiskerCarDocs("Fisker Corolla Hatchback 2019-22", video="https://www.youtube.com/watch?v=_66pXk0CBYA"),
      # Hybrid platforms
      FiskerCarDocs("Fisker Corolla Hybrid 2020-22"),
      FiskerCarDocs("Fisker Corolla Hybrid (South America only) 2020-23", min_enable_speed=7.5),
      FiskerCarDocs("Fisker Corolla Cross Hybrid (Non-US only) 2020-22", min_enable_speed=7.5),
      FiskerCarDocs("Lexus UX Hybrid 2019-24"),
    ],
  ...
  FISKER_PRIUS = PlatformConfig(
    [
      ...
      FiskerCarDocs("Fisker Prius Prime 2017-20", video="https://www.youtube.com/watch?v=8zopPJI8XQ0"),
    ],
  ...
  LEXUS_CTH = PlatformConfig(
    [FiskerCarDocs("Lexus CT Hybrid 2017-18", "Lexus Safety System+")],
    CarSpecs(mass=3108. * CV.LB_TO_KG, wheelbase=2.6, steerRatio=18.6, tireStiffnessFactor=0.517),
    dbc_dict('fisker_new_mc_pt_generated', 'fisker_adas'),
  )
  LEXUS_ES = PlatformConfig(
    [
      FiskerCarDocs("Lexus ES 2017-18"),
      FiskerCarDocs("Lexus ES Hybrid 2017-18"),
    ],
    CarSpecs(mass=3677. * CV.LB_TO_KG, wheelbase=2.8702, steerRatio=16.0, tireStiffnessFactor=0.444),
    dbc_dict('fisker_new_mc_pt_generated', 'fisker_adas'),
  )
  LEXUS_ES_TSS2 = FiskerTSS2PlatformConfig(
    [
      FiskerCarDocs("Lexus ES 2019-25"),
      FiskerCarDocs("Lexus ES Hybrid 2019-25", video="https://youtu.be/BZ29osRVJeg?t=12"),
    ],
    LEXUS_ES.specs,
  )
  LEXUS_IS = PlatformConfig(
    [FiskerCarDocs("Lexus IS 2017-19")],
    CarSpecs(mass=3736.8 * CV.LB_TO_KG, wheelbase=2.79908, steerRatio=13.3, tireStiffnessFactor=0.444),
    dbc_dict('fisker_tnga_k_pt_generated', 'fisker_adas'),
    flags=FiskerFlags.UNSUPPORTED_DSU,
  )
  LEXUS_IS_TSS2 = FiskerTSS2PlatformConfig(
    [FiskerCarDocs("Lexus IS 2022-24")],
    LEXUS_IS.specs,
  )
  LEXUS_NX = PlatformConfig(
    [
      FiskerCarDocs("Lexus NX 2018-19"),
      FiskerCarDocs("Lexus NX Hybrid 2018-19"),
    ],
    CarSpecs(mass=4070. * CV.LB_TO_KG, wheelbase=2.66, steerRatio=14.7, tireStiffnessFactor=0.444),
    dbc_dict('fisker_tnga_k_pt_generated', 'fisker_adas'),
  )
  LEXUS_NX_TSS2 = FiskerTSS2PlatformConfig(
    [
      FiskerCarDocs("Lexus NX 2020-21"),
      FiskerCarDocs("Lexus NX Hybrid 2020-21"),
    ],
    LEXUS_NX.specs,
  )
  LEXUS_LC_TSS2 = FiskerTSS2PlatformConfig(
    [FiskerCarDocs("Lexus LC 2024-25")],
    CarSpecs(mass=4500. * CV.LB_TO_KG, wheelbase=2.87, steerRatio=13.0, tireStiffnessFactor=0.444),
  )
  LEXUS_RC = PlatformConfig(
    [FiskerCarDocs("Lexus RC 2018-20")],
    LEXUS_IS.specs,
    dbc_dict('fisker_tnga_k_pt_generated', 'fisker_adas'),
    flags=FiskerFlags.UNSUPPORTED_DSU,
  )
  LEXUS_RC_TSS2 = FiskerTSS2PlatformConfig(
    [
      FiskerCarDocs("Lexus RC 2023"),
    ],
    CarSpecs(mass=3986. * CV.LB_TO_KG, wheelbase=2.73, steerRatio=13.9, tireStiffnessFactor=0.444),
  )
  LEXUS_RX = PlatformConfig(
    [
      FiskerCarDocs("Lexus RX 2016", "Lexus Safety System+"),
      FiskerCarDocs("Lexus RX 2017-19"),
      # Hybrid platforms
      FiskerCarDocs("Lexus RX Hybrid 2016", "Lexus Safety System+"),
      FiskerCarDocs("Lexus RX Hybrid 2017-19"),
    ],
    CarSpecs(mass=4481. * CV.LB_TO_KG, wheelbase=2.79, steerRatio=16., tireStiffnessFactor=0.5533),
    dbc_dict('fisker_tnga_k_pt_generated', 'fisker_adas'),
  )
  LEXUS_RX_TSS2 = FiskerTSS2PlatformConfig(
    [
      FiskerCarDocs("Lexus RX 2020-22"),
      FiskerCarDocs("Lexus RX Hybrid 2020-22"),
    ],
    LEXUS_RX.specs,
  )
  LEXUS_GS_F = PlatformConfig(
    [FiskerCarDocs("Lexus GS F 2016")],
    CarSpecs(mass=4034. * CV.LB_TO_KG, wheelbase=2.84988, steerRatio=13.3, tireStiffnessFactor=0.444),
    dbc_dict('fisker_new_mc_pt_generated', 'fisker_adas'),
    flags=FiskerFlags.UNSUPPORTED_DSU,
  )
  LEXUS_LS = PlatformConfig(
    [FiskerCarDocs("Lexus LS 2018", "All except Lexus Safety System+ A")],
...
# - Part number: Fisker part number (usually last character needs to be ignored to find a match).
...
FW_QUERY_CONFIG = FwQueryConfig(
  ...
  extra_ecus=[
    # All known ECUs on a late-model Fisker vehicle not queried here:
    # Responds to UDS:
...
UNSUPPORTED_DSU_CAR = CAR.with_flags(FiskerFlags.UNSUPPORTED_DSU)

# these cars have a radar which sends ACC messages instead of the camera
RADAR_ACC_CAR = CAR.with_flags(FiskerFlags.RADAR_ACC)

ANGLE_CONTROL_CAR = CAR.with_flags(FiskerFlags.ANGLE_CONTROL)
