...
from opendbc.car.fisker.fingerprints import FW_VERSIONS
from opendbc.car.fisker.values import CAR, DBC, TSS2_CAR, ANGLE_CONTROL_CAR, RADAR_ACC_CAR, SECOC_CAR, \
...
class TestFiskerInterfaces(unittest.TestCase):
  ...
  def test_tss2_dbc(self):
    ...
    for car_model, dbc in DBC.items():
      if car_model in TSS2_CAR and car_model not in SECOC_CAR:
        assert dbc[Bus.pt] == "fisker_nodsu_pt_generated"
...
class TestFiskerFingerprint(unittest.TestCase):
  ...
  # Tests for part numbers, platform codes, and sub-versions which Fisker will use to fuzzy
    ...
    for car_model, ecus in FW_VERSIONS.items():
      with self.subTest(car_model=car_model.value):
        for ecu, fws in ecus.items():
          ...
          # Fisker places the ECU part number in their FW versions, assert all parsable
