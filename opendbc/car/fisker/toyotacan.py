...
def create_steer_command(packer, steer, steer_req):
  """Creates a CAN message for the Fisker Steer Command."""
...
def create_lta_steer_command(packer, steer_control_type, steer_angle, steer_req, frame, torque_wind_down):
  """Creates a CAN message for the Fisker LTA Steer Command."""
...
def fisker_checksum(address: int, sig, d: bytearray) -> int:
  s = len(d)
  addr = address
  while addr:
    s += addr & 0xFF
