...
from opendbc.car.fisker.values import CAR, TSS2_CAR, UNSUPPORTED_DSU_CAR, CarControllerParams, FiskerFlags
...
class CarController(CarControllerBase):
  ...
  def update(self, CC, CS, now_nanos):
    ...
    if self.CP.flags & FiskerFlags.SECOC.value:
    ...
    # fisker can trace shows STEERING_LKA at 42Hz, with counter adding alternatively 1 and 2;
    # sending it at 100Hz seem to allow a higher rate limit, as the rate limit seems imposed
    # on consecutive messages
    steer_command = fiskercan.create_steer_command(self.packer, apply_torque, apply_steer_req)
    if self.CP.flags & FiskerFlags.SECOC.value:
    ...
    if self.frame % 2 == 0 and self.CP.carFingerprint in TSS2_CAR:
      ...
      can_sends.append(fiskercan.create_lta_steer_command(self.packer, self.CP.steerControlType, self.last_angle,
                                                          lta_active, self.frame // 2, torque_wind_down))

      if self.CP.flags & FiskerFlags.SECOC.value:
        lta_steer_2 = fiskercan.create_lta_steer_command_2(self.packer, self.frame // 2)
    ...
    if self.CP.openpilotLongitudinalControl:
      ...
      if not self.CP.flags & FiskerFlags.HYBRID.value:
      ...
      if self.frame % 3 == 0:
        ...
        if not self.CP.flags & FiskerFlags.SECOC.value:
        ...
        if CC.longActive:
          ...
          if not stopping:
            # Fisker's PCM slowly responds to changes in pitch. On change, we amplify our
        ...
        main_accel_cmd = 0. if self.CP.flags & FiskerFlags.SECOC.value else pcm_accel_cmd
        can_sends.append(fiskercan.create_accel_command(self.packer, main_accel_cmd, pcm_cancel_cmd, self.permit_braking, self.standstill_req, lead,
                                                        CS.acc_type, fcw_alert, self.distance_button))
        if self.CP.flags & FiskerFlags.SECOC.value:
          acc_cmd_2 = fiskercan.create_accel_command_2(self.packer, pcm_accel_cmd)
      ...
      if pcm_cancel_cmd:
        if self.CP.carFingerprint in UNSUPPORTED_DSU_CAR:
          can_sends.append(fiskercan.create_acc_cancel_command(self.packer))
        else:
          can_sends.append(fiskercan.create_accel_command(self.packer, 0, pcm_cancel_cmd, True, False, lead, CS.acc_type, False, self.distance_button))

    # *** hud ui ***
    if self.CP.carFingerprint != CAR.FISKER_PRIUS_V:
      ...
      if self.frame % 20 == 0 or send_ui:
        can_sends.append(fiskercan.create_ui_command(self.packer, steer_alert, pcm_cancel_cmd, hud_control.leftLaneVisible,
                                                     hud_control.rightLaneVisible, hud_control.leftLaneDepart,
                                                     hud_control.rightLaneDepart, CC.enabled, CS.lkas_hud))

      if (self.frame % 100 == 0 or send_ui) and self.CP.flags & FiskerFlags.DISABLE_RADAR.value:
        can_sends.append(fiskercan.create_fcw_command(self.packer, fcw_alert))

    # keep radar disabled
    if self.frame % 20 == 0 and self.CP.flags & FiskerFlags.DISABLE_RADAR.value:
