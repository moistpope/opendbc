...
from opendbc.car.fisker.values import Ecu, CAR, DBC, FiskerFlags, CarControllerParams, TSS2_CAR, RADAR_ACC_CAR, MIN_ACC_SPEED, \
                                                  EPS_SCALE, ANGLE_CONTROL_CAR, FiskerSafetyFlags
...
class CarInterface(CarInterfaceBase):
  ...
  def _get_params(ret: structs.CarParams, candidate, fingerprint, car_fw, alpha_long, is_release, docs) -> structs.CarParams:
    ...
    if DBC[candidate][Bus.pt] == "fisker_new_mc_pt_generated":
      ret.safetyConfigs[0].safetyParam |= FiskerSafetyFlags.ALT_BRAKE.value

    if ret.flags & FiskerFlags.SECOC.value:
    ...
    if candidate in ANGLE_CONTROL_CAR:
      ...
      ret.safetyConfigs[0].safetyParam |= FiskerSafetyFlags.LTA.value
    ...
    if candidate in RADAR_ACC_CAR:
      ...
      if alpha_long:
        ret.flags |= FiskerFlags.DISABLE_RADAR.value
    ...
    ret.openpilotLongitudinalControl = (candidate in (TSS2_CAR - RADAR_ACC_CAR) or
                                        bool(ret.flags & FiskerFlags.DISABLE_RADAR.value))

    ret.autoResumeSng = ret.openpilotLongitudinalControl

    if not ret.openpilotLongitudinalControl:
      ret.safetyConfigs[0].safetyParam |= FiskerSafetyFlags.STOCK_LONGITUDINAL.value
    ...
    if candidate in TSS2_CAR:
      ret.flags |= FiskerFlags.RAISED_ACCEL_LIMIT.value
      ...
      if ret.flags & FiskerFlags.HYBRID.value:
  ...
  def init(CP, can_recv, can_send, communication_control=None):
    ...
    if CP.flags & FiskerFlags.DISABLE_RADAR.value:
